...
const multiparty_upload = function multiparty_upload(req, auto) {
  ...
  // 自动上传会将文件上传到 uploadDir 目录下
  if (auto) config.uploadDir = uploadDir
  return new Promise(async (resolve, reject) => {
    //  延迟函数用于模拟服务器处理，前端能有明显的loading效果，真实业务场景不需要 
    await delay()
    // multiparty.Form(config).parse() 方法将客户端传的 FormData 格式的数据按照 config配置 进行解析
...
app.post('/upload_single_base64', async (req, res) => {
  ...
  // 除去base64的声明部分，获取真正的内容部分
  file = file.replace(/^data:image\/\w+;base64,/, '')
  // 使用node提供的 Buffer.from() 将 base64 变成 buffer 数据
  file = Buffer.from(file, 'base64')
  //   将文件添加到  spark
  spark.append(file)
  // 通过 spark.end() 方法获取到根据文件内容生成的 hash 名称
